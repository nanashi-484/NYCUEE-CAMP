...
void ReadButton(){
  for (int i = 0; i < buttonCount; i++) {
    ...
    if (currentState && !lastButtonStates[i]) {
      ...
      // butten_shortPress[i] = 1; // 短按先記下
    }

    else if (!currentState && lastButtonStates[i]) {
      // 按鈕剛放開
      if (!longPressDetected[i]) {
        butten_shortPress[i] = 1; // 如果沒被當成長按，當作短按
      }
      else {
        butten_shortPress[i] = 0; // 長按已處理，不再視為短按
      }
    }
...
void receiveCommand() {
  ...
  /*else*/ if (butten_shortPress[0] == 1) command_current = PLAY;
  else if (butten_shortPress[1] == 1) command_current = STOP;
  //else if (longPressDetected[2] == 1) command_current = SET_REC;
  else if (butten_shortPress[2] == 1) command_current = REC;
  else if (longPressDetected[3] == 1) command_current = G_ERASE; 
  else if (butten_shortPress[3] == 1) command_current = ERASE;
  else if (butten_shortPress[4] == 1) command_current = FWD;
  else if (butten_shortPress[5] == 1) command_current = G_ERASE;
